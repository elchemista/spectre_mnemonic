if Code.ensure_loaded?(Spectre.Instance.Ref) do
  defmodule Spectre.Mnemonic.Memory do
    ...
    def recall(cue, opts) when is_list(opts) do
      with {:ok, agent} <- agent(opts),
           {:ok, memory_opts} <-
             options(agent, Keyword.put(opts, :mnemonic_operation_kind, :recall)) do
    ...
    alias Spectre.Stack.Runtime, as: StackRuntime
    ...
    def remember(payload, opts) when is_list(opts) do
      with {:ok, agent} <- agent(opts),
           {:ok, memory_opts} <-
             options(agent, Keyword.put(opts, :mnemonic_operation_kind, :remember)) do
    ...
    def remember(%Input{} = input, %Result{} = result, agent, opts)
      ...
      runtime_opts =
        Keyword.merge(opts,
          ...
          result: result,
          agent: agent,
          mnemonic_operation_kind: :remember
        )
    ...
    def options(agent, runtime_opts \\ []) when is_atom(agent) and is_list(runtime_opts) do
      with {:ok, config} <- Spectre.Mnemonic.config(agent) do
        ...
        with {:ok, options} <- normalize_agent_ref(options, agent),
          ...
          configure_runtime_engine(options, config)
        end
      end
    end

    @spec configure_runtime_engine(keyword(), map()) :: {:ok, keyword()} | {:error, term()}
    defp configure_runtime_engine(opts, config) do
      case Keyword.get(opts, :stack_runtime) do
        nil ->
          {:ok, configure_store(opts, Map.get(config, :store))}

        runtime when is_atom(runtime) and not is_nil(runtime) ->
          resolve_stack_engine(runtime, opts, config)

        {:via, module, _term} = runtime when is_atom(module) ->
          resolve_stack_engine(runtime, opts, config)

        runtime ->
          {:error, {:invalid_mnemonic_stack_runtime, runtime}}
      end
    end

    @spec resolve_stack_engine(GenServer.name(), keyword(), map()) ::
            {:ok, keyword()} | {:error, term()}
    defp resolve_stack_engine(runtime, opts, config) do
      with owner when is_atom(owner) and not is_nil(owner) <- Map.get(config, :stack_owner),
           {:ok, resource_ref} <- Spectre.Stack.resolve(owner, :resource, :engine),
           {:ok, engine} <- StackRuntime.resolve(runtime, resource_ref) do
        {:ok,
         opts
         |> Keyword.put(:engine, engine)
         |> Keyword.drop([:persistent_memory, :stores, :data_root, :embedding, :secret_crypto])}
      else
        nil -> {:error, :mnemonic_stack_owner_required}
        {:error, _reason} = error -> error
        _invalid -> {:error, :mnemonic_stack_owner_required}
      end
    end
    ...
    @spec put_operation_id(keyword()) :: keyword()
    defp put_operation_id(opts) do
      if Keyword.has_key?(opts, :operation_id) do
        opts
      else
        Keyword.put(opts, :operation_id, derive_operation_id(opts))
      end
    end

    @spec derive_operation_id(keyword()) :: binary()
    defp derive_operation_id(opts) do
      metadata = Keyword.get(opts, :run_metadata, %{})
      instance_ref = field(metadata, :instance_ref)
      result_metadata = opts |> Keyword.get(:result) |> field(:metadata)
      run = field(metadata, :run) || field(result_metadata, :run)
      run_id = Keyword.get(opts, :run_id) || field(metadata, :run_id) || field(run, :id)

      revision =
        Keyword.get(opts, :committed_state_revision) ||
          field(metadata, :committed_state_revision) || field(run, :revision)

      step_id = Keyword.get(opts, :step_id) || field(metadata, :step_id) || field(run, :step_id)

      logical = {
        field(instance_ref, :schema_version),
        field(instance_ref, :key),
        run_id,
        revision,
        step_id,
        Keyword.get(opts, :mnemonic_operation_kind, :memory)
      }

      if Enum.any?([run_id, revision, step_id], &valid_dimension_value?/1) do
        digest =
          logical
        ...
        SpectreMnemonic.Identity.uuid7()
      end
    end
    ...
    defp dimension(:instance, opts) do
      case opts |> Keyword.get(:run_metadata) |> field(:instance_ref) do
        %Spectre.Instance.Ref{} = ref -> {ref.schema_version, ref.key}
        _missing -> nil
      end
    end
    ...
    defp record(agent, event, result, opts) do
      ...
      isolation =
        case Keyword.get(opts, :scope) do
          {:spectre, partition} when is_list(partition) -> Keyword.keys(partition)
          _shared -> []
        end
