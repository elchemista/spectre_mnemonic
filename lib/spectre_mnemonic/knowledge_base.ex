defmodule SpectreMnemonic.KnowledgeBase do
  @moduledoc """
  ...
  def append(event, opts \\ []), do: KnowledgeSMEM.append(event, opts)

  @doc "Replays compact knowledge events."
  @spec events(keyword()) :: {:ok, [map()]}
  def events(opts \\ []), do: KnowledgeSMEM.replay(opts)

  @doc "Loads a compact, budgeted knowledge packet."
  @spec load(keyword()) :: {:ok, Knowledge.t()}
  def load(opts \\ []) do
    cfg = config(opts)

    if Keyword.get(cfg, :enabled, true) do
      {:ok, events} = KnowledgeSMEM.replay(cfg)
  ...
  @spec build_packet([map()], keyword()) :: Knowledge.t()
  ...
  @spec empty_packet(map()) :: Knowledge.t()
