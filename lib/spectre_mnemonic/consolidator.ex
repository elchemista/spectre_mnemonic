defmodule SpectreMnemonic.Consolidator do
  ...
  defp persist_plan(plan, now) do
    results =
      Enum.map(plan.moments, &PersistentMemory.append(:moments, &1)) ++
        Enum.map(plan.knowledge, &PersistentMemory.append(:knowledge, &1)) ++
        Enum.map(plan.summaries, &PersistentMemory.append(:summaries, &1)) ++
        Enum.map(plan.categories, &PersistentMemory.append(:categories, &1)) ++
        Enum.map(plan.embeddings, &PersistentMemory.append(:embeddings, &1)) ++
        Enum.map(plan.associations, &PersistentMemory.append(:associations, &1)) ++
        Enum.map(plan.records, fn {family, payload} ->
          PersistentMemory.append(family, payload)
        end) ++
        [
  ...
  @spec embedding_record(SpectreMnemonic.Moment.t()) :: [map()]
