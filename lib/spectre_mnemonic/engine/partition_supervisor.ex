defmodule SpectreMnemonic.Engine.PartitionSupervisor do
  @moduledoc false
  ...
  def init(%Config{} = config) do
    case Registry.register(@registry, {:partition_supervisor, config.ref}, nil) do
      {:ok, _owner} ->
        DynamicSupervisor.init(strategy: :one_for_one)

      {:error, {:already_registered, pid}} ->
        raise "partition supervisor already started for #{inspect(config.ref)}: #{inspect(pid)}"
    end
  end
end
