defmodule SpectreMnemonicKnowledgeSMEMTest do
  ...
  test "knowledge smem appends and replays compact events" do
    ...
    assert {:ok, events} = KnowledgeSMEM.replay()
  ...
  test "load knowledge respects item and byte budgets" do
    ...
    {:ok, _} = KnowledgeSMEM.append(%{type: :summary, summary: "Compact project memory"})
    {:ok, _} = KnowledgeSMEM.append(%{type: :skill, name: "first", steps: ["a"]})
    {:ok, _} = KnowledgeSMEM.append(%{type: :skill, name: "second", steps: ["b"]})
    {:ok, _} = KnowledgeSMEM.append(%{type: :latest_ingestion, text: "newest note"})
    {:ok, _} = KnowledgeSMEM.append(%{type: :latest_ingestion, text: "older note"})
    {:ok, _} = KnowledgeSMEM.append(%{type: :fact, text: "fact one"})
    {:ok, _} = KnowledgeSMEM.append(%{type: :fact, text: "fact two"})
  ...
  test "search knowledge returns targeted scored events without loading active moments" do
    {:ok, _} = KnowledgeSMEM.append(%{type: :summary, summary: "Global compact memory"})

    {:ok, _} =
      KnowledgeSMEM.append(%{type: :fact, text: "Postgres adapter stores vector metadata"})

    {:ok, _} =
      KnowledgeSMEM.append(%{type: :skill, name: "sqlite-debug", steps: ["inspect jobs"]})

    before_count = length(SpectreMnemonic.Focus.moments())
    ...
    assert length(SpectreMnemonic.Focus.moments()) == before_count
  end
  ...
  test "recall includes compact knowledge without hydrating active moments" do
    {:ok, _} = KnowledgeSMEM.append(%{type: :summary, summary: "Recall should attach this"})
    {:ok, _} = KnowledgeSMEM.append(%{type: :fact, text: "durable compact fact"})
    ...
    assert [%SpectreMnemonic.Knowledge{summary: "Recall should attach this"}] = packet.knowledge
    assert length(SpectreMnemonic.Focus.moments()) == before_count
  end

  test "old knowledge structs with text only still work" do
    knowledge = %SpectreMnemonic.Knowledge{
      id: "know_old",
      source_id: "mom_1",
  ...
  defmodule CustomCompactAdapter do
    @behaviour SpectreMnemonic.Compact.Adapter
